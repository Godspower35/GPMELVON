...
class PaymentRequest(models.Model):
    ...
    def action_pay(self):
        ...
        for record in self:
            ...
            # Partner validation
            partner_id = record.requester_id.partner_id if record.requester_id else False
            if not partner_id:
                raise exceptions.UserError(_("Please specify a requester with valid partner!"))
            ...
            # Create a description for the credit line from all line descriptions
            credit_description = "; ".join([line.name for line in record.request_line if line.name])
            
            # Create credit line with minimal info
            credit_vals = {
                "name": credit_description,  # Use concatenated line descriptions
                "move_id": move.id,
            ...
            move.action_post()
            _logger.info(f"Posted journal entry with ID: {move.id}")
